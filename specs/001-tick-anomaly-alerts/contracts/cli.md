...
- Reads JSONL tick fixtures, a partitioned tick directory, or a merged daily JSONL generated by `partition-ticks`.
...
## Command: partition-ticks

```bash
tick-stream partition-ticks --input var/replay/history_ticks_20260624_20260626.jsonl --out-dir var/replay/ticks --merged-dir var/replay/merged
```

Behavior:

- Reads a source JSONL tick file.
- Writes symbol-level files under `trading_date=YYYY-MM-DD/SYMBOL.jsonl`.
- Writes one `manifest.json` per trading date with symbol counts, file paths, and first/last event times.
- Writes merged daily JSONL files under `--merged-dir` so existing full-day replay workflows remain simple.
- Supports repeated `--date YYYY-MM-DD` filters when only selected trading dates should be partitioned.
