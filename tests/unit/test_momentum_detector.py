...
def test_momentum_detector_ignores_tiny_jump_after_flat_baseline():
    config = load_config("tests/fixtures/config/valid_watchlist.yml")
    engine = DetectionEngine(config)
    events = []
    for i, price in enumerate([100, 100, 100, 100, 100, 100.1]):
        _, evts, _ = engine.process_raw({"symbol": "SHSE.600519", "event_time": f"2026-06-25T10:00:0{i}+08:00", "last_price": price})
        events.extend(evts)
    assert not any(event.anomaly_type == AnomalyType.MOMENTUM_SPIKE for event in events)
